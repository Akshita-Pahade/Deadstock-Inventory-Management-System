...
-- Complete Database Setup Script  (FINAL VERSION)
-- Run this ONCE in MySQL before starting Flask
-- ══════════════════════════════════════════════════════════════════

CREATE DATABASE IF NOT EXISTS deadstock_db
...
DROP TABLE IF EXISTS DELETED_WAREHOUSE;
...
-- ─────────────────────────────────────────────────────────────────
-- DEADSTOCK
-- IMPORTANT: ON DELETE RESTRICT on Branch_ID so deadstock rows
-- are NEVER automatically deleted when a branch is removed.
-- app.py handles all deadstock state before deleting the branch.
-- ─────────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────────
-- REPORT  (one per branch; READ-ONLY for Branch Head)
-- All 6 counters updated by app.py allocate() route
-- Disposal → Items_Disposed +qty, Estimated_Waste_Reduced unchanged
-- ─────────────────────────────────────────────────────────────────
CREATE TABLE REPORT (
...
-- ─────────────────────────────────────────────────────────────────
DROP TRIGGER IF EXISTS trg_update_branch_rating;
...
-- Mumbai warehouse must exist for reassign logic
...
-- ─────────────────────────────────────────────────────────────────
-- IF YOU ALREADY HAVE AN EXISTING DATABASE, run only these:
-- ─────────────────────────────────────────────────────────────────
-- ALTER TABLE REPORT ADD COLUMN IF NOT EXISTS Items_Upcycled  INT NOT NULL DEFAULT 0;
-- ALTER TABLE REPORT ADD COLUMN IF NOT EXISTS Items_Rebranded INT NOT NULL DEFAULT 0;
-- ALTER TABLE REPORT ADD COLUMN IF NOT EXISTS Items_Disposed  INT NOT NULL DEFAULT 0;
--
-- -- Fix FK on DEADSTOCK to prevent auto-delete on branch deletion:
-- ALTER TABLE DEADSTOCK DROP FOREIGN KEY <your_fk_name>;
...
-- ══════════════════════════════════════════════════════════════════
-- ALTER STATEMENTS — Branch Deletion Fix + All Change Sets
-- Run once against deadstock_db
-- ══════════════════════════════════════════════════════════════════
USE deadstock_db;

-- ─────────────────────────────────────────────────────────────────
-- STEP 1 (CRITICAL FIX): Change DEADSTOCK.Branch_ID FK to ON DELETE SET NULL
--
-- WHY: The original FK was ON DELETE RESTRICT, which blocks DELETE FROM BRANCH
--      whenever any DEADSTOCK row still references that Branch_ID.
--      Since we intentionally keep deadstock rows alive after branch deletion
--      (they get moved to the warehouse), the FK must allow the branch row to
--      be removed without cascading deletions.
--
--      ON DELETE SET NULL: When a BRANCH row is deleted, Branch_ID in DEADSTOCK
--      becomes NULL automatically. The DELETED_BRANCH archive table preserves
--      all branch history, and LEFT JOIN queries use DELETED_BRANCH for reporting.
-- ─────────────────────────────────────────────────────────────────

-- 1a. Find the current FK name on DEADSTOCK.Branch_ID:
...
-- 1b. Drop the old FK (replace 'deadstock_ibfk_1' with the actual name from above):
-- ALTER TABLE DEADSTOCK DROP FOREIGN KEY deadstock_ibfk_1;

-- 1c. Re-add with ON DELETE SET NULL:
-- ALTER TABLE DEADSTOCK
--   ADD CONSTRAINT fk_deadstock_branch
--   FOREIGN KEY (Branch_ID) REFERENCES BRANCH(Branch_ID) ON DELETE SET NULL;

-- Verify:
-- SHOW CREATE TABLE DEADSTOCK;
-- Should show: CONSTRAINT ... FOREIGN KEY (`Branch_ID`) REFERENCES `BRANCH` (`Branch_ID`) ON DELETE SET NULL

-- ─────────────────────────────────────────────────────────────────
-- STEP 2: ADMIN Email column for OTP forgot-password flow
-- ─────────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────────
-- STEP 3: One-head-one-branch / one-head-one-warehouse
-- ─────────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────────
-- STEP 4: Python install (run in terminal, not MySQL)
--   pip install reportlab
-- ─────────────────────────────────────────────────────────────────

-- ─────────────────────────────────────────────────────────────────
-- STEP 5: DELETED_BRANCH table (if not yet created)
--   This table archives branch rows on deletion so reports & deadstock
--   queries can still look up branch info via LEFT JOIN DELETED_BRANCH.
-- ─────────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────────
-- STEP 6: DELETED_WAREHOUSE table (if not yet created)
-- ─────────────────────────────────────────────────────────────────
CREATE TABLE IF NOT EXISTS DELETED_WAREHOUSE (
...
SELECT 'All ALTER statements applied successfully.' AS Status;
...
CREATE TABLE RECYCLE_RECORD (
  ...
  Sustainability_Quality ENUM('High','Medium','Low'),
  Allocated_At    TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (Recycle_ID),
  FOREIGN KEY (Deadstock_ID) REFERENCES DEADSTOCK(Deadstock_ID) ON DELETE CASCADE
);

-- New RECYCLE_RECORD table
CREATE TABLE IF NOT EXISTS RECYCLE_RECORD (
  ...
  PRIMARY KEY (Recycle_ID),
  FOREIGN KEY (Deadstock_ID)
    REFERENCES DEADSTOCK(Deadstock_ID) ON DELETE CASCADE
);



DELIMITER $$
CREATE TRIGGER trg_insert_recycle_record
AFTER INSERT ON STOCK_ALLOCATION
FOR EACH ROW
BEGIN
  IF NEW.Allocation_Type = 'Recycle' THEN
    INSERT INTO RECYCLE_RECORD
      (Deadstock_ID, Branch_ID, Category, Size,
       Material_ID, Quantity, Material_Name, Sustainability_Quality, Allocated_At)
    SELECT d.Deadstock_ID, d.Branch_ID, d.Category, d.Size,
           d.Material_ID, d.Quantity, m.Material_Name,
           m.Sustainability_Quality, NEW.Allocated_At
    FROM DEADSTOCK d
    JOIN MATERIAL m ON d.Material_ID = m.Material_ID
    WHERE d.Deadstock_ID = NEW.Deadstock_ID;
  END IF;
END$$
DELIMITER ;
...
-- ══════════════════════════════════════════════════════════════════
-- FEATURE UPGRADES MIGRATION (run on existing databases)
-- Safe to re-run: ignores duplicate column/table/trigger errors
-- ══════════════════════════════════════════════════════════════════
USE deadstock_db;

-- SKU column on DEADSTOCK
SET @col = (SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS
            WHERE TABLE_SCHEMA='deadstock_db' AND TABLE_NAME='DEADSTOCK' AND COLUMN_NAME='SKU');
SET @sql = IF(@col = 0,
  'ALTER TABLE DEADSTOCK ADD COLUMN SKU VARCHAR(50) UNIQUE AFTER Deadstock_ID',
  'SELECT ''SKU column already exists'' AS msg');
PREPARE stmt FROM @sql; EXECUTE stmt; DEALLOCATE PREPARE stmt;

-- Date_Added column on DEADSTOCK
SET @col = (SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS
            WHERE TABLE_SCHEMA='deadstock_db' AND TABLE_NAME='DEADSTOCK' AND COLUMN_NAME='Date_Added');
SET @sql = IF(@col = 0,
  'ALTER TABLE DEADSTOCK ADD COLUMN Date_Added DATE NOT NULL DEFAULT (CURDATE()) AFTER Quantity',
  'SELECT ''Date_Added column already exists'' AS msg');
PREPARE stmt FROM @sql; EXECUTE stmt; DEALLOCATE PREPARE stmt;

-- Backfill SKU for existing rows
UPDATE DEADSTOCK SET SKU = CONCAT(
  UPPER(LEFT(REPLACE(Category,' ','-'),6)), '-', Branch_ID, '-', Size, '-',
  YEAR(COALESCE(Date_Added, CURDATE())), LPAD(Deadstock_ID, 4, '0')
...
-- Trigger to record rating changes
DROP TRIGGER IF EXISTS trg_rating_history;
DELIMITER $$
CREATE TRIGGER trg_rating_history
AFTER UPDATE ON BRANCH
FOR EACH ROW
BEGIN
  IF NEW.Sustainable_Rating != OLD.Sustainable_Rating OR OLD.Sustainable_Rating IS NULL THEN
    INSERT INTO RATING_HISTORY (Branch_ID, Rating, Recorded_At)
    VALUES (NEW.Branch_ID, NEW.Sustainable_Rating, CURDATE());
  END IF;
END$$
DELIMITER ;

-- Seed initial rating history from current branch ratings
INSERT INTO RATING_HISTORY (Branch_ID, Rating, Recorded_At)
SELECT Branch_ID, Sustainable_Rating, CURDATE()
FROM BRANCH
WHERE Sustainable_Rating IS NOT NULL
  AND Branch_ID NOT IN (SELECT Branch_ID FROM RATING_HISTORY);
